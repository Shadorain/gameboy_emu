...
static void proc_nop (cpu_context *ctx) { /* does nothing */ }
...
static void proc_cp (cpu_context *ctx) {
    ...
    cpu_set_flags(ctx, n == 0, 1, ((int)ctx->regs.a & 0x0F) -
        ((int)ctx->fetched_data & 0x0F) < 0, n < 0);
}
...
static void proc_ld (cpu_context *ctx) {
    if (ctx->dest_is_mem) {
        ...
        if (is_16bit(ctx->cur_inst->reg_2)) /* if 16 bit reg */
            emu_cycles(1);
...
static void proc_ldh (cpu_context *ctx) {
    ...
    else bus_write(ctx->mem_dest, ctx->regs.a);
    emu_cycles(1);
}
...
static void proc_inc (cpu_context *ctx) {
    ...
    if ((ctx->cur_opcode & 0x03) == 0x03) return;
...
static void proc_dec (cpu_context *ctx) {
    ...
    if ((ctx->cur_opcode & 0x0B) == 0x0B) return;
...
static void proc_add (cpu_context *ctx) {
    ...
    bool is_16b = is_16bit(ctx->cur_inst->reg_1);

    if (is_16b) emu_cycles(1);
